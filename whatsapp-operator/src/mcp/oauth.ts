...
export async function registerOAuthClient(req: Request, res: Response): Promise<void> {
  ...
  const redirectUris = Array.isArray(body.redirect_uris) ? body.redirect_uris.map(cleanString).filter(Boolean) : [];
  const tokenEndpointAuthMethod = cleanString(body.token_endpoint_auth_method) || 'none';
  const grantTypes = Array.isArray(body.grant_types) ? body.grant_types.map(cleanString) : ['authorization_code', 'refresh_token'];
  const responseTypes = Array.isArray(body.response_types) ? body.response_types.map(cleanString) : ['code'];

  if (redirectUris.length === 0 || redirectUris.some((uri) => !allowedRedirectUri(uri))) {
    res.status(400).json({ error: 'invalid_client_metadata', error_description: 'redirect_uris must contain valid HTTPS URLs (localhost is allowed only outside production)' });
    return;
  }
  if (!grantTypes.includes('authorization_code') || !responseTypes.includes('code')) {
    res.status(400).json({ error: 'invalid_client_metadata', error_description: 'authorization_code/code are required' });
    return;
  }
  ...
  const clientSecret = tokenEndpointAuthMethod === 'none' ? null : randomToken(32);
  const clientSecretHash = clientSecret ? hashToken(clientSecret) : null;

  await pool.query(
    `INSERT INTO mcp_oauth_clients (client_id, client_name, redirect_uris, client_secret_hash, token_endpoint_auth_method, created_at)
     VALUES ($1, $2, $3, $4, $5, now())`,
    [clientId, clientName, redirectUris, clientSecretHash, tokenEndpointAuthMethod],
  );
...
function authorizeError(res: Response, params: { redirectUri: string; error: string; description?: string; state?: string }): void {
  ...
  if (params.description) url.searchParams.set('error_description', params.description);
...
async function getClient(clientId: string): Promise<{ client_id: string; client_name: string; redirect_uris: string[]; client_secret_hash: string | null; token_endpoint_auth_method: string } | null> {
  const result = await pool.query(
    `SELECT client_id, client_name, redirect_uris, client_secret_hash, token_endpoint_auth_method
...
export async function oauthAuthorize(req: Request, res: Response): Promise<void> {
  ...
  const clientId = cleanString(query.client_id);
  const redirectUri = cleanString(query.redirect_uri);
  const responseType = cleanString(query.response_type);
  const codeChallenge = cleanString(query.code_challenge);
  const codeChallengeMethod = cleanString(query.code_challenge_method);
  ...
  if (responseType !== 'code' || !codeChallenge || codeChallengeMethod !== 'S256') {
    authorizeError(res, { redirectUri, error: 'invalid_request', description: 'Authorization code + PKCE S256 are required', state });
    return;
  }
  if (!config.mcpOauthPassword) {
    res.status(503).send('Remote MCP OAuth is not configured. Set MCP_OAUTH_PASSWORD on the Operator.');
    return;
  }
  ...
  if (username !== config.mcpOauthUsername || !safeEqual(password, config.mcpOauthPassword)) {
  ...
  await pool.query(
    `INSERT INTO mcp_oauth_codes (code_hash, client_id, redirect_uri, code_challenge, scope, expires_at, created_at)
     VALUES ($1, $2, $3, $4, $5, now() + ($6 * interval '1 second'), now())`,
    [hashToken(code), clientId, redirectUri, codeChallenge, requestedScopes.join(' '), AUTH_CODE_TTL_SECONDS],
  );
...
function clientSecretFromRequest(req: Request): string {
  const authorization = cleanString(req.header('authorization'));
  ...
  return cleanString(req.body?.client_secret);
}
...
async function authenticateClient(req: Request, clientId: string): Promise<boolean> {
  ...
  return safeEqual(hashToken(clientSecretFromRequest(req)), client.client_secret_hash);
}

export async function oauthToken(req: Request, res: Response): Promise<void> {
  const grantType = cleanString(req.body?.grant_type);
  const clientId = cleanString(req.body?.client_id);
  if (!clientId || !(await authenticateClient(req, clientId))) {
  ...
  if (grantType === 'authorization_code') {
    ...
    const redirectUri = cleanString(req.body?.redirect_uri);
    const codeVerifier = cleanString(req.body?.code_verifier);
    ...
    const result = await pool.query(
      `DELETE FROM mcp_oauth_codes
       WHERE code_hash = $1 AND client_id = $2 AND redirect_uri = $3 AND expires_at > now() AND used_at IS NULL
       RETURNING code_challenge, scope`,
    ...
    const verifierChallenge = createHash('sha256').update(codeVerifier).digest('base64url');
    if (!safeEqual(verifierChallenge, String(row.code_challenge))) {
      res.status(400).json({ error: 'invalid_grant' });
      return;
    }
  ...
  if (grantType === 'refresh_token') {
    const refreshToken = cleanString(req.body?.refresh_token);
    if (!refreshToken) {
      res.status(400).json({ error: 'invalid_request' });
      return;
    }
    const result = await pool.query(
      `SELECT client_id, scope, revoked_at, refresh_expires_at
         FROM mcp_oauth_tokens WHERE refresh_token_hash = $1`,
      [hashToken(refreshToken)],
    );
    const tokenRow = result.rows[0];
    if (!tokenRow || tokenRow.revoked_at || new Date(tokenRow.refresh_expires_at).getTime() <= Date.now()) {
      res.status(400).json({ error: 'invalid_grant' });
      return;
    }
    await pool.query(`UPDATE mcp_oauth_tokens SET revoked_at = now() WHERE refresh_token_hash = $1`, [hashToken(refreshToken)]);
    return issueTokens(res, clientId, String(tokenRow.scope));
  }
...
async function issueTokens(res: Response, clientId: string, scope: string): Promise<void> {
  ...
  await pool.query(
    `INSERT INTO mcp_oauth_tokens (access_token_hash, refresh_token_hash, client_id, app_id, tenant_id, scope, access_expires_at, refresh_expires_at, created_at)
     VALUES ($1, $2, $3, $4, $5, $6, now() + ($7 * interval '1 second'), now() + ($8 * interval '1 second'), now())`,
    [hashToken(accessToken), hashToken(refreshToken), clientId, config.mcpAppId, config.mcpTenantId, scope, ACCESS_TOKEN_TTL_SECONDS, REFRESH_TOKEN_TTL_SECONDS],
  );
...
export async function verifyAccessToken(token: string): Promise<{ clientId: string; appId: string; tenantId: string; scopes: string[]; expiresAt: number } | null> {
  const result = await pool.query(
    `SELECT client_id, app_id, tenant_id, scope, access_expires_at, revoked_at
       FROM mcp_oauth_tokens WHERE access_token_hash = $1`,
    [hashToken(token)],
  );
  const row = result.rows[0];
  if (!row || row.revoked_at) return null;
  const expiresAtMs = new Date(row.access_expires_at).getTime();
...
export function bearerChallenge(): string {
  return `Bearer realm="NahaLabs WhatsApp MCP", resource_metadata="${publicBaseUrl()}/.well-known/oauth-protected-resource/mcp"`;
}
